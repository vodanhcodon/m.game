...
/**
 * Thực thể được tạo bởi lớp UploadHandler, sẽ tự động in ra 1 chuỗi JSON trả về 
 * khi khởi tạo kết thúc
 * Mô hình xử lý như sau:
 * __construct --> initialize --> $this->post() --> $this->generate_response
 * --> $this->body($json);
 */
class UploadHandler {
    ...
    protected $error_messages = array(
        1 => 'Kích thước file được upload vượt quá thiết lập upload_max_filesize trong php.ini',
        2 => 'Kích thước file được upload vượt quá thiết lập MAX_FILE_SIZE được định nghĩa trong HTML form',
        3 => 'File được tải lên bị lỗi',
        4 => 'Không có file nào được tải lên',
        6 => 'Thiếu temporary folder',
        7 => 'Không ghi được file vào ổ cứng',
        8 => '1 PHP extension đã ngăn chặn quá trình file upload',
        'post_max_size' => 'Kích thước file được upload vượt quá thiết lập post_max_size trong php.ini',
        'max_file_size' => 'Kích thước file quá lớn',
        'min_file_size' => 'Kích thước file quá nhỏ',
        'accept_file_types' => 'Định dạng file không đúng',
        'max_number_of_files' => 'Vượt quá tối đa số file được cho phép tải lên',
        'max_width' => 'File ảnh vượt quá width giới hạn',
        'min_width' => 'File ảnh có yêu cầu width tối thiểu',
        'max_height' => 'File ảnh vượt quá height giới hạn',
        'min_height' => 'File ảnh có yêu cầu height tối thiểu'
    );

    /**
     * __construct method
     * @param string $action_link - đường dẫn tới response action trong Controller
     * @param type $options
     * @param type $initialize
     * @param type $error_messages
     */
    function __construct($action_link, $options = null, $initialize = true, $error_messages = null) {
        $this->get_full_url($action_link);
        $this->options = array(
            'script_url' => $action_link . '/',
            'upload_dir' => Configure::read('gom.tmp'),
            'upload_url' => Configure::read('gom.tmp'),
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function get_full_url($path) {
...
//            substr($_SERVER['SCRIPT_NAME'],0, strrpos($_SERVER['SCRIPT_NAME'], '/'));
    ...
    public function delete($print_response = true, $detele_link = NULL) {
        if (empty($detele_link)) {
            $file_name = $this->get_file_name_param();
            $file_path = $this->get_upload_path($file_name);
