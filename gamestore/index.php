...
switch ($act) {
    case 'download':
        ...
        break;
    case 'detail':
        $game_app = mysql_query("
            SELECT * FROM `gom_game_app` as `a` WHERE `a`.`id` = '" . $id . "'
                ");
...
//                <tr>
//                    <td width="30%">
//                       <img src="' . $set['homeurl'] . '/gameapp_files/' . $res['name'] . '/' . $res['logo'] . '" />
//                    </td>';
//                echo '
//                    <td width="70%" valign="top">
//                    <a href="' . $set['homeurl'] . '/gamestore/index.php?src=' . $src . '&amp;act=download&amp;id=' . $res['j2me_jad_file_path'] . '">Tải miễn phí</a><br /><br />' 
//                    . $res['total_download'] . ' số lượt tải<br /><br />';
//                echo '
...
//            echo '<tr><td colspan="2"><hr /></td></tr>';
                functions::detail_item_game_app($res, $src, $deviceInfo, $set);
            }
            $cate_relation = mysql_query("
                SELECT 
                      `a`.`id`, `a`.`name` 
                FROM `gom_the_loai` as `a` 
                WHERE 
                      `a`.`id` IN (
                                   SELECT 
                                         `b`.`the_loai_id`
                ...
                while (($res_cate_relation = mysql_fetch_assoc($cate_relation)) !== false) {
                    ...
                    $str_query = "
                        SELECT 
							  ...
							  `b`.`short_decription`
                        FROM `gom_the_loai_relation` as `a`
						INNER JOIN `gom_game_app` as `b` ON `a`.`game_app_id` = `b`.`id`
						INNER JOIN `gom_the_loai` as `c` ON `a`.`the_loai_id` = `c`.`id`
						WHERE 
                              `c`.`id` = " . $res_cate_relation['id'] . " 
                        AND 
                              `b`.`id` != " . $id . " 
                        AND 
                               LOWER(`b`.`device_support`) LIKE '%" . $deviceInfo['deviceModel'] . "%' 
                        AND
                               `b`.status = 2
                        ORDER BY 
                              `b`.`last_update` 
                    ...
                    while (($res_top5 = mysql_fetch_assoc($lst_game_app_top5)) !== false) {
//                        if (isset($res_top5['logo'])) {
//                            echo '<tr><td width="30%"><a href="' . $set['homeurl'] . '/gamestore/index.php?src=' . $src . '&amp;act=detail&amp;id=' . $res_top5['id'] . '"><img src="' . $set['homeurl'] . '/images/' . $res_top5['logo'] . '" /></a></td>';
//                            echo '<td width="70%" valign="top">' . $res_top5['name'] . '<br />' . $res_top5['j2me_jad_file_path'] . '<br />' . $res_top5['id'] . '<br /><div class="chitiet"><a href="' . $set['homeurl'] . '/gamestore/index.php?src=' . $src . '&amp;act=detail&amp;id=' . $res_top5['id'] . '">Chi tiết</a></div></td></tr>';
//                        } else {
//                            echo '<tr><td colspan="2" valign="top">' . $res_top5['name'] . '<br />' . $res_top5['j2me_jad_file_path'] . '<br />' . $res_top5['short_decription'] . '<br /><div class="chitiet"><a href="' . $set['homeurl'] . '/gamestore/index.php?src=' . $src . '&amp;act=detail&amp;id=' . $res_top5['id'] . '">Chi tiết</a></div></td></tr>';
//                        }
//                        echo '<tr><td colspan="2"><hr /></td></tr>';
                        functions::thumbnail_item_game_app($res_top5, $src, $deviceInfo, $set);
                    }
                }
            }
        }
    ...
    case 'category':
        echo '<tr height="8px;"><td colspan="2"></td></tr>';
        $danhmuc = mysql_query("
            SELECT `a`.`name` FROM `gom_danh_muc` as `a` WHERE `a`.`id` = " . $id
        );
        if (mysql_num_rows($danhmuc)) {
            //max displayed per page
            $per_page = 9;
            // count records
            $count_record = mysql_num_rows(mysql_query("
                SELECT 
                      `a`.`id` 
            ...
            while (($res_category = mysql_fetch_assoc($danhmuc)) !== false) {
                ...
                echo '<tr height="8px;"><td colspan="2"></td></tr>';
                $lst_game_app_top9_in_danhmuc = mysql_query("
                    SELECT 
                          `a`.`id`, `a`.`logo`, `a`.`name`, `a`.`j2me_jad_file_path`, 
...
//                        }
//                        echo '<tr><td colspan="2"><hr /></td></tr>';
                        functions::thumbnail_item_game_app($res_game_app_top9, $src, $deviceInfo, $set);
                    }
                }
                /*
                  -----------------------------------------------------------------
    ...
    case 'type':
        ...
        if (mysql_num_rows($theloai)) {
            ...
            $count_record = mysql_num_rows(mysql_query("
                SELECT 
                    `a`.`id` FROM `gom_the_loai_relation` as `b`
				INNER JOIN 
                    `gom_game_app` as `a` ON `b`.`game_app_id` = `a`.`id`
				WHERE 
                    `b`.`the_loai_id` = " . $id . " ORDER BY `a`.`last_update`
                AND 
                     LOWER(`a`.`device_support`) LIKE '%" . $deviceInfo['deviceModel'] . "%' 
                AND
                     `a`.status = 2
            ...
            while (($res_type = mysql_fetch_assoc($theloai)) !== false) {
                ...
                $lst_game_app_top9_in_theloai = mysql_query("
                    SELECT 
                          `a`.`id`, `a`.`logo`, `a`.`name`, `a`.`j2me_jad_file_path`,
						  `a`.`j2me_jar_file_path`, `a`.`android_apk_file_path`, `a`.`short_decription`
					FROM 
                          `gom_the_loai_relation` as `b`
					INNER JOIN 
                          `gom_game_app` as `a` ON `b`.`game_app_id` = `a`.`id`
					WHERE 
                          `b`.`the_loai_id` = " . $id . " 
                    ...
                    AND
                          `a`.status = 2
                    ORDER BY 
                          `a`.`last_update` LIMIT " . $per_page * ($current_page - 1) . ", " . $per_page);
...
//                     <tr>
//                        <td width="30%">
//                           <a href="' . $set['homeurl'] . '/gamestore/index.php?src=' . $src . '&amp;act=detail&amp;id=' . $res['id'] . '">
//                            <img src="' . $set['homeurl'] . '/images/' . $res['logo'] . '" />
//                           </a>
//                        </td>';
...
//                            echo '<a href="' . $set['homeurl'] . '/' . $res['j2me_jar_file_path'] . '">Giá: ' . $res['price'] . ' đồng</a>' . '<br />';
//                        } elseif ($deviceInfo['deviceOS'] == 'android') {
//                            echo '<a href="' . $set['homeurl'] . '/' . $res['android_apk_file_path'] . '">Giá: ' . $res['price'] . ' đồng</a>' . '<br />';
//                        } else {
//                            echo '<a href="#">Không hỗ trợ</a>' . '<br />';
//                        }
//                    } else {
//                        if ($deviceInfo['j2meSupport']) {
//                            echo '<a href="' . $set['homeurl'] . '/' . $res['j2me_jar_file_path'] . '">Tải miễn phí</a>' . '<br />';
//                        } elseif ($deviceInfo['deviceOS'] == 'android') {
//                            echo '<a href="' . $set['homeurl'] . '/' . $res['android_apk_file_path'] . '">Tải miễn phí</a>' . '<br />';
//                        } else {
//                            echo '<a href="#">Không hỗ trợ</a>' . '<br />';
//                        }
//                    }
//                    echo $res['short_decription'] . '<br />
//                          <div class="chitiet">
//                                <a href="' . $set['homeurl'] . '/gamestore/index.php?src=' . $src . '&amp;act=detail&amp;id=' . $res['id'] . '">Chi tiết</a>
//                          </div>
//                        </td>
//                     </tr>';
//                } else {
...
//                }
//                echo '<tr><td colspan="2"><hr /></td></tr>';
                functions::thumbnail_item_game_app($res, $src, $deviceInfo, $set);
            }
        }
        /*
          -----------------------------------------------------------------
...
//        if (isset($advanced_danhmuc) && ($advanced_danhmuc != ''))
        ...
        // count records
        $count_record = mysql_num_rows(mysql_query("
            SELECT `a`.`id` FROM `gom_game_app` as `a` " . $inner_join . $where
        ));
        // count max pages
        $max_pages = ceil($count_record / $per_page);
        // current page
...
//                            </a>
//                            </td>';
//                    echo '<td width="70%" valign="top">' . $res['name'] . '<br />';
//                    if (!empty($res['price'])) {
//                        if ($deviceInfo['j2meSupport']) {
//                            echo '<a href="' . $set['homeurl'] . '/' . $res['j2me_jar_file_path'] . '">Giá: ' . $res['price'] . ' đồng</a>' . '<br />';
//                        } elseif ($deviceInfo['deviceOS'] == 'android') {
...
//                    echo '
//                          <tr>
//                            <td colspan="2" valign="top">' . $res['name'] . '<br />'
//                    . $res['j2me_jad_file_path'] . '<br />'
//                    . $res['short_decription'] .
//                    '</td>
//                          </tr>';
//                }
//                echo '<tr><td colspan="2"><hr /></td></tr>';
...
//                echo '<tr><td colspan="2"><hr /></td></tr>';
    ...
    case 'topevent':
        ...
        $lst_result = mysql_query("
            SELECT 
                  `a`.`id`, `a`.`name`, `a`.`short_decription`, `a`.`last_update`
			FROM `gom_game_event` as `a` 
            WHERE
                  `a`.status = 2
            ORDER BY  
                 `a`.`last_update` DESC 
    ...
    case 'eventdetail':
        ...
        $event_game = mysql_query("
            SELECT 
                  `a`.`name`, `a`.`event_forum_link`, `a`.`game_forum_link`,
                  `a`.`last_update`, `a`.`description`, `a`.`game_app_id`,
        ...
        if (mysql_num_rows($event_game)) {
            while (($res_event = mysql_fetch_assoc($event_game)) !== false) {
                ...
                $game_app = mysql_query("
                    SELECT 
                          `a`.`name`, `a`.`logo`, `a`.`j2me_jad_file_path`, 
                          `a`.`j2me_jar_file_path`,
						  `a`.`total_download` 
                    FROM `gom_game_app` as `a` 
                    WHERE 
                         `a`.status = 2
        ...
        $lst_game_app_top3 = mysql_query("
            ...
            AND
                  LOWER(`a`.`device_support`) LIKE '%" . $deviceInfo['deviceModel'] . "%'
            ORDER BY `a`.`total_download` DESC 
            LIMIT 3");
        while (($res = mysql_fetch_assoc($lst_game_app_top3)) !== false) {
//            if (isset($res['logo'])) {
//                echo '<tr><td width="30%"><a href="' . $set['homeurl'] . '/gamestore/index.php?src=' . $src . '&amp;act=detail&amp;id=' . $res['id'] . '"><img src="' . $set['homeurl'] . '/gameapp_files/' . $res['name'] . '/' . $res['logo'] . '" /></a></td>';
//                echo '<td width="70%" valign="top">' . $res['name'] . '<br />' . $res['j2me_jad_file_path'] . '<br />' . $res['short_decription'] . '<br /><div class="chitiet"><a href="' . $set['homeurl'] . '/gamestore/index.php?src=' . $src . '&amp;act=detail&amp;id=' . $res['id'] . '">Chi tiết</a></div></td></tr>';
//            } else {
