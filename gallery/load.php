...
if ((!empty($_SESSION['uid']) && $rz1['user'] == 1 && $ms['text'] == $login) || $rights >= 6) {
    ...
    if ($fname != "") {
        ...
        if ((preg_match("/php/i", $ffail)) or (preg_match("/.pl/i", $fname)) or ($fname == ".htaccess")) {
            echo $lng_gal['prohibited_file'] .".<br/><a href='index.php?act=upl&amp;id=" . $id . "'>" . $lng['repeat'] . "</a><br/>";
            require_once('../incfiles/end.php');
            exit;
        }
        if ($fsize >= 1024 * $ftsz) {
            echo $lng_gal['file_size_exceeds'] ." $ftsz kB<br/><a href='index.php?act=upl&amp;id=" . $id . "'>" . $lng['repeat'] . "</a><br/>";
            require_once('../incfiles/end.php');
            exit;
        }
        if (!in_array($formfail, $dopras)) {
            echo $lng_gal['file_allowed'] .": $tff !.<br/><a href='index.php?act=upl&amp;id=" . $id . "'>" . $lng['repeat'] . "</a><br/>";
            require_once('../incfiles/end.php');
            exit;
        }
        if (preg_match("/[^\da-zA-Z_\-.]+/", $fname)) {
            echo $lng_gal['invalid_character'] ."<br/><a href='index.php?act=upl&amp;id=" . $id . "'>" . $lng['repeat'] . "</a><br/>";
            require_once('../incfiles/end.php');
            exit;
        }
        ...
        if ((move_uploaded_file($_FILES["fail"]["tmp_name"], "foto/$fname")) == true) {
            ...
            @chmod("$ch", 0777);
            @chmod("foto/$ch", 0777);
            echo $lng_gal['image_uploaded'] ."!<br/><a href='index.php?id=" . $id . "'>" . $lng_gal['to_album'] . "</a><br/>";
