...
class core {
    ...
    public static $pdo;                                                        // thuộc tính lưu trữ đối tượng truy cập DB: PDO
    ...
    function __construct() {
        global $rootpath;
        if (isset($rootpath))
            self::$root = $rootpath;                          // Задаем путь к корневой папке
    ...
    public static function load_lng($module = '_core') {
        if (!is_dir(self::$root . 'incfiles/languages/' . self::$lng_iso))
            self::$lng_iso = 'en';
    ...
    /*
      ...
      -----------------------------------------------------------------
      Hàm lấy về thông tin thiết bị hiện tại dựa vào $user_agent
      @param string $user_agent
      @return nếu xác định được trả về 1 mảng array,
      nếu không xác định được trả về FALSE
     */

    public static function getDeviceInfo($user_agent = null) {
        // nhúng mã nguồn TeraWurfl
        require(self::$root . 'incfiles/vendor/TeraWurfl/TeraWurfl.php');

        $user_agent = isset($user_agent) ? $user_agent : self::$user_agent;
...
//            $deviceModel = $brandName . ' ' . $modelName;
            ...
            return array(
                'deviceModel' => strtolower($deviceModel), 'j2meSupport' => $j2meSupport,
    ...
    /**
      -----------------------------------------------------------------
      khởi tạo đối tượng twig object, dùng để tạo template và caching
      -----------------------------------------------------------------
     */
    public static function getTwig() {
        // nhúng vào thư viện Twig
        require(self::$root . 'incfiles/vendor/twig/twig/lib/Twig/Autoloader.php');
        Twig_Autoloader::register();

        // nhúng vào đường dẫn thư mục chứa templates
        $loader = new Twig_Loader_Filesystem(self::$root . 'templates/gom');

        // đường dẫn tới thư mục cache
        $cache = self::$root . 'templates/cache';
